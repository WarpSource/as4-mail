...
import si.laurentius.msh.dao.NamedQueries;
import si.laurentius.msh.dao.QueryCriteriaBuilder;
...
@WebService(serviceName = "mailbox", portName = "SEDMailBoxWSPort", endpointInterface = "si.laurentius.SEDMailBoxWS", targetNamespace = "http://laurentius.si", wsdlLocation = "WEB-INF/wsdl/mailbox.wsdl")
public class SEDMailBox implements SEDMailBoxWS {
    ...
    private static final String MSG_ERR_QUEUE_CONF = "Error occurred while submitting mail to ebms queue. Check queue configuration: factory: '%s' queue: '%s'.";
    ...
    SimpleDateFormat msdfDDMMYYYYHHMMSS = new SimpleDateFormat("dd.MM.yyyy HH:mm:ss");
    ...
    public String getAuthenticatedPrincipal() {
        return mwsCtxt != null && mwsCtxt.getUserPrincipal() != null ? mwsCtxt.getUserPrincipal().getName() : null;
    }
    ...
    public GetInMailResponse getInMail(GetInMailRequest param) throws SEDException_Exception {
        ...
        if (dt.getReceiverEBox() == null) {
            throw SEDRequestUtils.createSEDException(String.format(MSG_ERR_MISSING_ATTR, PARAMETER_RECEIVER_EBOX), SEDExceptionCode.MISSING_DATA);
        }

        validatePrincipalBox(dt.getReceiverEBox());

        if (dt.getMailId() == null) {
            throw SEDRequestUtils.createSEDException(String.format(MSG_ERR_MISSING_ATTR, "MailId"), SEDExceptionCode.MISSING_DATA);
        }
        // init response
        ...
        try {
            ...
            cq.where(cb.and(cb.equal(om.get(PARAMETER_RECEIVER_EBOX), dt.getReceiverEBox()), cb.equal(om.get("Id"), dt.getMailId())));
            ...
            String message = String.format(MSG_ERR_MISSING_MAIL, param.getData().getMailId().toString(), param.getData().getReceiverEBox());
    ...
    private byte[] getContentStorageForFilePath(String filePath) {
        try {
            ...
            LOG.formatedWarning("Error occurred while retrieving filepath %s content. Error:", filePath, ex.getMessage());
        }
        return null;
    ...
    public InMailEventListResponse getInMailEventList(InMailEventListRequest param) throws SEDException_Exception {
        ...
        TypedQuery<InEvent> q = memEManager.createNamedQuery(NamedQueries.LAU_NQ_INMAIL_GET_EVENTS, InEvent.class);
    ...
    public InMailListResponse getInMailList(InMailListRequest intMailListRequest) throws SEDException_Exception {
        ...
        if (data == null) {
            throw SEDRequestUtils.createSEDException("Empty data in request: InMailList/Data", SEDExceptionCode.MISSING_DATA);
        }
        ...
        try {
            QueryCriteriaBuilder criteriaBuilder = new QueryCriteriaBuilder(memEManager.getCriteriaBuilder(),InMail.class,data )
                    .sortField(c.getSortBy())
                    .sortOrder(c.getSortOrder());

            CriteriaQuery<Long> cqCount = criteriaBuilder.buildForCount();
            CriteriaQuery<InMail> cq = criteriaBuilder.build();
    ...
    public OutMailEventListResponse getOutMailEventList(OutMailEventListRequest param) throws SEDException_Exception {
        ...
        TypedQuery<OutEvent> q = memEManager.createNamedQuery(NamedQueries.LAU_NQ_OUTMAIL_GET_EVENTS, OutEvent.class);
    ...
    public OutMailListResponse getOutMailList(OutMailListRequest outMailListRequest) throws SEDException_Exception {
        ...
        try {
            QueryCriteriaBuilder criteriaBuilder = new QueryCriteriaBuilder(memEManager.getCriteriaBuilder(),OutMail.class,data )
                    .sortField(c.getSortBy())
                    .sortOrder(c.getSortOrder());

            CriteriaQuery<Long> cqCount = criteriaBuilder.buildForCount();
            CriteriaQuery<OutMail> cq = criteriaBuilder.build();
    ...
    private OutMail mailExists(OutMail mail) {
        ...
        TypedQuery<OutMail> q = memEManager.createNamedQuery(NamedQueries.LAU_NQ_OUTMAIL_getByMessageIdAndSenderBox, OutMail.class);
    ...
    public ModifyInMailResponse modifyInMail(ModifyInMailRequest param) throws SEDException_Exception {
        ...
        if (dt.getAction() == null) {
            throw SEDRequestUtils.createSEDException("Invalid or missing action!", SEDExceptionCode.INVALID_DATA);
        }
        ...
        try {
            ...
            if (SEDInboxMailStatus.RECEIVED.getValue().equals(im.getStatus()) || SEDInboxMailStatus.LOCKED.getValue().equals(im.getStatus()) || SEDInboxMailStatus.ERROR.getValue().equals(im.getStatus())) {
                ...
                if (st == null) {
                    String message = String.format("Action  %s is not supported !", dt.getAction().value());
                ...
                String message = String.format("Mail with id '%s' and  ebox: %s is in invalid status %s !", param.getData().getMailId().toString(), param.getData().getReceiverEBox(), im.getStatus());
                throw SEDRequestUtils.createSEDException(message, SEDExceptionCode.REQUIRED_DATA_NOT_EXISTS);
            }
    ...
    public ModifyOutMailResponse modifyOutMail(ModifyOutMailRequest param) throws SEDException_Exception {
        ...
        try {
            ...
            cq.where(cb.and(cb.equal(om.get("senderEBox"), dt.getSenderEBox()), cb.equal(om.get("id"), dt.getMailId())));
            ...
            switch (dt.getAction()) {
                ...
                case DELETE:
                    if (omail.getStatus().equalsIgnoreCase(SEDOutboxMailStatus.SUBMITTED.getValue()) || omail.getStatus().equalsIgnoreCase(SEDOutboxMailStatus.ERROR.getValue()) || omail.getStatus().equalsIgnoreCase(SEDOutboxMailStatus.FAILED.getValue()) || omail.getStatus().equalsIgnoreCase(SEDOutboxMailStatus.SCHEDULE.getValue())) {
                        ...
                        rsp.getRData().setOutEvent(ou);
                    } else if (omail.getStatus().equalsIgnoreCase(SEDOutboxMailStatus.SENT.getValue()) || omail.getStatus().equalsIgnoreCase(SEDOutboxMailStatus.PUSHING.getValue())) {
                        throw SEDRequestUtils.createSEDException("Sent mail or mail in progress can not be DELETED", SEDExceptionCode.INVALID_DATA);
                    }
                    break;
                case RESEND:
                    if (omail.getStatus().equalsIgnoreCase(SEDOutboxMailStatus.ERROR.getValue()) || omail.getStatus().equalsIgnoreCase(SEDOutboxMailStatus.FAILED.getValue()) || omail.getStatus().equalsIgnoreCase(SEDOutboxMailStatus.DELETED.getValue())) {

                        omail.setStatus(SEDOutboxMailStatus.SCHEDULE.getValue());
                        omail.setStatusDate(Calendar.getInstance().getTime());

                        OutEvent ou = setOutMailStatus(omail, "Resending by user/application: " + c.getUserId() + "/" + c.getApplicationId(), c.getUserId(), c.getApplicationId());
                        ...
                        try {
                            ...
                            setOutMailStatus(omail, "Error occurred while resending mail: " + c.getUserId() + "/" + c.getApplicationId() + " Error: " + se.getMessage(), c.getUserId(), c.getApplicationId());
                            throw se;
                        }
                    } else {
            ...
            String message = String.format(MSG_ERR_MISSING_MAIL, param.getData().getMailId().toString(), param.getData().getSenderEBox());
            throw SEDRequestUtils.createSEDException(message, SEDExceptionCode.REQUIRED_DATA_NOT_EXISTS);
        }
        return rsp;
    }

    private void serializeMail(OutMail mail, String userID, String applicationId, PMode pmode) throws SEDException_Exception {
        ...
        try {
            ...
            throw new SEDException_Exception("Error occurred while storing payload", msherr, ex);
        }
        ...
        try {
            // create JMS session
            cf = InitialContext.doLookup(msgFactoryJndiName);
            if (mqMSHQueue == null) {
            ...
            msherr.setErrorCode(SEDExceptionCode.SERVER_ERROR);
            msherr.setMessage(ex.getMessage());
            throw new SEDException_Exception(String.format(MSG_ERR_QUEUE_CONF, msgFactoryJndiName, msgQueueJndiName), msherr, ex);
        }

        try (Connection connection = cf.createConnection(); Session session = connection.createSession(true, Session.SESSION_TRANSACTED); MessageProducer sender = session.createProducer(mqMSHQueue)) {
            ...
            ut.commit();
            session.commit();
            LOG.formatedlog("Transaction commited: user %s, appl %s, pmodeId %s", userID, applicationId, pmode.getId());
            ...
            msherr.setErrorCode(SEDExceptionCode.SERVER_ERROR);
            msherr.setMessage(ex.getMessage());
            throw new SEDException_Exception("Error occurred while storing to DB", msherr, ex);
    ...
    private void addMailToSubmitQueue(OutMail mail) throws SEDException_Exception {
        long l = LOG.logStart();
        // --------------------
        // serialize data and submit message
        String msgFactoryJndiName = getJNDIPrefix() + SEDValues.EBMS_JMS_CONNECTION_FACTORY_JNDI;
        ...
        try (Connection connection = cf.createConnection(); Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE); MessageProducer sender = session.createProducer(mqMSHQueue);) {
    ...
    public SubmitMailResponse submitMail(SubmitMailRequest submitMailRequest) throws SEDException_Exception {
        ...
        if (sb == null) {
            String msg = MSG_ERR_SENDER_BOX + mail.getSenderEBox() + " not exists";
            ...
            if (sb.getActiveFromDate() != null && sb.getActiveFromDate().after(Calendar.getInstance().getTime())) {
                String msg = MSG_ERR_SENDER_BOX + mail.getSenderEBox() + " is  active! (Activation from : '" + sb.getActiveFromDate().toString() + "')";
                throw SEDRequestUtils.createSEDException(msg, SEDExceptionCode.INVALID_DATA);
            }
            if (sb.getActiveToDate() != null && sb.getActiveToDate().before(Calendar.getInstance().getTime())) {
                String msg = MSG_ERR_SENDER_BOX + mail.getSenderEBox() + " is  active! (Activation To : '" + sb.getActiveToDate().toString() + "')";
                throw SEDRequestUtils.createSEDException(msg, SEDExceptionCode.INVALID_DATA);
            }
        }
        ...
        if (om == null) {
            ...
            serializeMail(mail, submitMailRequest.getControl().getUserId(), submitMailRequest.getControl().getApplicationId(), pmd);

        }
        // generate response
        SubmitMailResponse rsp = new SubmitMailResponse();
        rsp.setRControl(new RControl());
        rsp.getRControl().setReturnValue(om != null || !lstWarn.isEmpty() ? SVEVReturnValue.WARNING.getValue() : SVEVReturnValue.OK.getValue());
        rsp.getRControl().setReturnText((om != null ? String.format("Mail with sender message id '%s' already sent before: %s", om.getSenderMessageId(), msdfDDMMYYYYHHMMSS.format(om.getSubmittedDate())) : "") + String.join(", ", lstWarn));
        // set data
    ...
    private PMode validateOutMailData(OutMail mail) throws SEDException_Exception {
        ...
        try {
            ...
            String sendingRole = Objects.equals(act.getInvokeRole(), ActionRole.Executor.getValue()) ? srv.getExecutor().getRole() : srv.getInitiator().getRole();

            pm = mpModeManager.getPModeForLocalPartyAsSender(sPID.getId(), sendingRole, rPID.getId(), mail.getService());
        } catch (PModeException ex) {
            throw SEDRequestUtils.createSEDException(ex.getMessage(), SEDExceptionCode.INVALID_DATA);
        }
        return pm;
    }
    ...
    public void validateControl(Control c) throws SEDException_Exception {
        ...
        if (c.getApplicationId() == null) {
            throw SEDRequestUtils.createSEDException("SubmitMailRequest/Control/@applicationId", SEDExceptionCode.MISSING_DATA);
        }
        if (c.getUserId() == null) {
        ...
        if (!Objects.equals(c.getApplicationId(), getAuthenticatedPrincipal())) {
            throw SEDRequestUtils.createSEDException(String.format("applicationId %s does not match authenticated principal %s ", c.getApplicationId(), getAuthenticatedPrincipal()), SEDExceptionCode.MISSING_DATA);
        }
    }
    ...
    public void validatePrincipalBox(String box) throws SEDException_Exception {
        ...
        if (Utils.isEmptyString(box)) {
            throw SEDRequestUtils.createSEDException("EBox is required attribute", SEDExceptionCode.MISSING_DATA);
        }
        String authenticatedId = getAuthenticatedPrincipal();
        if (Utils.isEmptyString(authenticatedId)) {
            throw SEDRequestUtils.createSEDException("Application auhtentication is required!", SEDExceptionCode.MISSING_DATA);
        }

        SEDApplication cms = mdbLookups.getSEDApplicationById(authenticatedId);
        if (cms == null) {
            throw SEDRequestUtils.createSEDException(String.format("Application [%s] is not registred!", authenticatedId), SEDExceptionCode.MISSING_DATA);

        }
        if (SEDRequestUtils.isNotValidMailAddress(box)) {
            throw SEDRequestUtils.createSEDException(String.format("box [%s] is not valid address!", box), SEDExceptionCode.MISSING_DATA);
        }
        ...
        throw SEDRequestUtils.createSEDException(String.format("Box [%s] is not assigned to application [%s]", box, authenticatedId), SEDExceptionCode.MISSING_DATA);
